...
int main()
{
	...
	Obstacle * obstacles = (Obstacle *) malloc(sizeof(Obstacle) * 3);
	int rng;
	Bike b;
	int frame=0;
	int newX;
	...
	while(1){
		vid_vsync();
		rng = qran_range(70,150);

		
        key_poll();
		if(((frame & 31) == 0) && colliding == 0){
			score++;
			posprintf(score_str, "score: %d", score);
			m3_clrs(140, 0, score_str,RGB15(0,0,0));
			m3_puts(140, 0, score_str ,RGB15(14,15,31));
			clearObstacles(obstacles,3);
			moveObstacles(obstacles,3);
			drawObstacles(obstacles,3);
			for (int i = 0; i<3; i++){
